...
export default function GeminiAnalysis({
  ...
  return (
    <div className="glass-panel p-5 border-purple-500/25 bg-slate-950/70">
      <div className="flex items-center justify-between border-b border-slate-900 pb-3 mb-4">
        <div className="flex items-center gap-2">
          <Sparkles className="text-purple-400 animate-pulse" size={18} />
          <h4 className="text-sm font-bold text-purple-400">Análise de IA (Gemini)</h4>
        </div>
      ...
      {loadingAnalysis && (
        <div className="text-center py-8">
          ...
          <p className="text-xs text-slate-350 font-bold mb-1">Mapeando estratégias...</p>
      ...
      {analysisError && !loadingAnalysis && (
        <div className="p-4 bg-red-950/20 border border-red-500/30 rounded-lg text-center">
          <AlertTriangle className="text-red-500 mx-auto mb-2" size={24} />
          <p className="text-xs font-bold text-red-400 mb-1">Erro na Análise</p>
          <p className="text-[11px] text-slate-450 mb-3">{analysisError}</p>
          <button 
            onClick={onStartAnalysis}
        ...
        <div className="markdown-content text-left text-xs max-h-[450px] overflow-y-auto pr-2 scrollbar-thin">
          {analysisResult.split('\n').map((line, i) => {
            ...
            if (trimmed.startsWith('### ')) {
              return (
                <h3 key={i} className="text-xs font-extrabold mt-5 mb-2 text-purple-400 border-b border-slate-900 pb-1 uppercase tracking-wider">
                  {trimmed.replace('### ', '')}
                </h3>
              );
            }
            if (trimmed.startsWith('## ')) {
              return (
                <h3 key={i} className="text-sm font-extrabold mt-5 mb-2 text-purple-400 border-b border-slate-900 pb-1 uppercase tracking-wider">
                  {trimmed.replace('## ', '')}
                </h3>
              );
            }
            if (trimmed.startsWith('- **Retirar:**') || trimmed.startsWith('- **Adicionar:**') || trimmed.startsWith('- **Motivo:**')) {
              ...
              return (
                <p key={i} className="ml-4 text-slate-300 my-1 leading-relaxed">
                  {boldMatch ? <strong className="text-purple-300 font-bold">{boldMatch[1]}</strong> : null}
                  {restText}
                </p>
              );
            }
            if (trimmed.startsWith('- ')) {
              return (
                <li key={i} className="ml-4 text-slate-400 list-disc my-1.5 leading-relaxed">
                  {trimmed.replace('- ', '')}
                </li>
              );
            }
            ...
            return (
              <p key={i} className="my-1.5 text-slate-400 leading-relaxed">
                {trimmed}
              </p>
            );
          })}
